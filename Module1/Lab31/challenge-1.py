...
dictt={'zero':0,'one':1,'two':2,'three':3,'four':4,'five':5}

letters=list(dictt.keys())

results={'negative five':-5,'negative four':-4,'negative three':-3,'negative two':-2,'negative one':-1,'zero':0,'one':1,'two':2,'three':3,'four':4,'five':5,'six':6,'seven':7,'eight':8,'nine':9,'ten':10}
...
if a in letters and c in letters:
    if b=='plus':
        soma=dictt[a]+dictt[c]
        position = results_values.index(soma)
        results=results_keys[position]
        print(f'{a} plus {b} equals {results}')
    elif b=='minus':
        soma=dictt[a]-dictt[c]
        position = results_values.index(soma)
        results=results_keys[position]
        print(f'{a} minus {b} equals {results}')
    else:
        print("I am not able to answer this question. Check your input.")
